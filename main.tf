resource "aws_ram_resource_share" "subnet_sharing" {
  count                     = length(var.private_subnets_arns) >= 0 || length(var.public_subnets_arns) >= 0 ? 1 : 0
...
resource "aws_ram_resource_association" "private_subnets" {
  count              = length(var.private_subnets_arns)
  resource_arn       = var.private_subnets_arns[count.index]
  resource_share_arn = aws_ram_resource_share.subnet_sharing[0].arn
}

resource "aws_ram_resource_association" "public_subnets" {
  count              = length(var.public_subnets_arns)
  resource_arn       = var.public_subnets_arns[count.index]
  resource_share_arn = aws_ram_resource_share.subnet_sharing[0].arn
}